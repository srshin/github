...
<script>
	...
	function drawVisualization() {  
	  var urls= "${pageContext.request.contextPath}/onion/areaChart.do";
	  var jsonData = $.ajax({
		  url: urls,
          dataType: "json",
          async: false
 ...
 <tr>
   <td>${regionList}</td>
    <c:forEach items="${areaList}" var="areaList">	
     <c:if test="${regionList==areaList.region}"><td><fmt:formatNumber>${areaList.area}</fmt:formatNumber></td></c:if>
   </c:forEach>
 </tr>
</c:forEach>
</table>
