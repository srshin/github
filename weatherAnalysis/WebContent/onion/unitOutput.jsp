...
<script>
	...
	function drawVisualization() {  
	  var urls= "${pageContext.request.contextPath}/onion/unitOutputChart.do";
	  var jsonData = $.ajax({
		  url: urls,
          dataType: "json",
          async: false
...
<c:forEach items="${regionList}" var="regionList">
 <tr>
   <td>${regionList}</td>
    <c:forEach items="${unitOutputList}" var="unitOutputList">	
     <c:if test="${regionList==unitOutputList.region}"><td><fmt:formatNumber>${unitOutputList.unitOutput}</fmt:formatNumber></td></c:if>
   </c:forEach>
 </tr>
</c:forEach>
</table>
